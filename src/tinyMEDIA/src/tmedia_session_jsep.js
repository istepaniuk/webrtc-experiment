    ...
    if (!this.o_sdp_lo && !this.b_sdp_lo_pending) {
        ...
        if (this.e_type == tmedia_type_e.AUDIO && (this.b_cache_stream && __o_jsep_stream_audio)) {
            ...
            if (!this.b_lo_held && !this.o_local_stream) {
                ...
                ivan_audio = (this.e_type == tmedia_type_e.SCREEN_SHARE) ? false : !!(this.e_type.i_id & tmedia_type_e.AUDIO.i_id); // IMPORTANT: Chrome '28.0.1500.95 m' doesn't support using audio with screenshare
                ivan_audio.mandatory = {
                    echoCancellation: false,
                    echoCancellation: false,
